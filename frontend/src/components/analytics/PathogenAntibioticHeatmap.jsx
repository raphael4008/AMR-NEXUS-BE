  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-md border border-white/50 p-5 overflow-x-auto">
      ...
      <table className="min-w-full text-sm">
        <thead><tr><th>Pathogen</th>{matrix.antibiotics.map(ab => <th key={ab} className="px-2 py-1">{ab}</th>)}</tr></thead>
        <tbody>
          {matrix.pathogens.map(path => (
            <tr key={path}><td className="font-mono">{path}</td>{matrix.antibiotics.map(ab => {
