...
export default function Reports() {
  ...
  const exportExcel = () => {
    ...
    try {
      let sheetData = [];
      if (reportType === 'mdr_summary') {
        sheetData = [
          ['Metric', 'Value'],
          ['Total Records', reportData.total_records || 0],
          ['MDR Rate (%)', reportData.mdr_rate || 0],
          ['Anomalies', reportData.anomaly_count || 0],
          ['Active Counties', reportData.active_counties || 0],
        ];
      } else if (reportType === 'sector_comparison' && reportData.sectors) {
        sheetData = [['Sector', 'MDR (%)'], ...reportData.sectors.map(s => [s.name, s.value])];
      } else if (reportType === 'county_ranking' && reportData.counties) {
        sheetData = [['County', 'MDR (%)'], ...reportData.counties.map(c => [c.county, c.rate])];
      } else if (reportType === 'pathogen_wise' && reportData.pathogens) {
        sheetData = [['Pathogen', 'Resistance (%)'], ...reportData.pathogens.map(p => [p.name, p.resistance])];
      } else if (reportType === 'trend' && reportData.trend) {
        sheetData = [['Month', 'MDR Rate (%)'], ...reportData.trend.map(t => [t.month, t.rate])];
      } else {
        toast.error('Unsupported report type for Excel export');
        return;
      }
  ...
  const exportCSV = () => {
    ...
    try {
      ...
      if (reportType === 'mdr_summary') {
        ...
        csvRows = [['Sector', 'MDR (%)'], ...reportData.sectors.map(s => [s.name, s.value])];
      } else if (reportType === 'county_ranking' && reportData.counties) {
        csvRows = [['County', 'MDR (%)'], ...reportData.counties.map(c => [c.county, c.rate])];
      } else if (reportType === 'pathogen_wise' && reportData.pathogens) {
        csvRows = [['Pathogen', 'Resistance (%)'], ...reportData.pathogens.map(p => [p.name, p.resistance])];
      } else if (reportType === 'trend' && reportData.trend) {
        csvRows = [['Month', 'MDR Rate (%)'], ...reportData.trend.map(t => [t.month, t.rate])];
      } else {
        toast.error('CSV export not supported for this report type');
        return;
      }
  ...
  const scheduleReport = async (email, schedule) => {
    ...
    try {
      ...
      toast.success(`Report scheduled ${schedule}ly to ${email}`);
      setShowScheduleModal(false);
    } catch (err) {
      toast.error('Failed to schedule. Ensure backend endpoint exists.');
    } finally {
      setScheduling(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Always render a container for PDF capture */}
      ...
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-2 px-5 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-full text-sm font-medium transition-colors"
        >
