...
class TestIntelligenceEndpoint:
    def test_dashboard_returns_200(self, client):
        resp = client.get("/api/v1/intelligence/dashboard")
        assert resp.status_code == 200
...
class TestOpenAPISchema:
    ...
    def test_openapi_has_intelligence_path(self, client):
        schema = client.get("/openapi.json").json()
        assert "/api/v1/intelligence/dashboard" in schema["paths"]

    def test_openapi_has_advisory_path(self, client):
        """Rev 2 advisory endpoint must be registered."""
        schema = client.get("/openapi.json").json()
        # FastAPI registers path templates with {alert_id}
        paths = schema["paths"]
        advisory_paths = [p for p in paths if "advisory" in p]
        assert len(advisory_paths) >= 1
